...
def collect_c(out_file, bk, obj_dir, directory, code_json_file):
    ...
    if bk == "tensorflow":
        use_lcov = 'PATH=~/local/bin/gcov-8.5:$PATH lcov -c --quiet --no-external {}{}--rc lcov_branch_coverage=1 --directory {} --base-directory {} -o {}'.format(
            exclude_str, include_str, obj_dir, directory, out_file)  # if we specify base directory, it is okay to add --no-external
    elif bk == "tensorflow_new" or bk == "pytorch":
        ...
        for i in include_list:
            include_str += '"' + i + '"' + ' '
        use_lcov = f'PATH=~/local/bin/gcov-9.4:$PATH python -m scripts.coverage.fastcov -b --jobs 16 --quiet --lcov {exclude_str}{include_str} --search-directory {obj_dir} --compiler-directory {directory} -o {out_file}'
    else:
        use_lcov = 'PATH=~/local/bin/gcov-5.5:$PATH lcov -c --quiet --no-external {}{}--rc lcov_branch_coverage=1 --directory {} --base-directory {} -o {}'.format(
...
class Coverage(Guidance):
    ...
    def collect_c_coverage(self, bk, python_bin, model_idntfr, config_dir, framework, library_path, silent):
        ...
        if bk in ["mxnet_new", "pytorch", "tensorflow_new"]:
            obj_dir = None
            if bk in ["mxnet", "pytorch", "mxnet_new"]:
                obj_dir = f"runtime/{bk}/"
            elif bk == "tensorflow_new":
                obj_dir = "runtime/tensorflow_new/k8-opt/bin/tensorflow/"
    ...
    def collect_coverage(self, bk, python_bin, model_idntfr, config_dir, framework, library_path, silent):
        ...
        try:
            ...
            info_content = self.collect_c_coverage(bk, python_bin, model_idntfr, config_dir, framework, library_path, silent)
            xml_content = self.collect_py_coverage(bk, python_bin, model_idntfr, config_dir, framework, library_path, silent)
