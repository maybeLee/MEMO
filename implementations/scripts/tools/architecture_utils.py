...
class ArchitectureUtils:
        ...
        for edge in edges:
            ...
            if assert_dim is True:
                if layer_class_dict is None:
                    raise Exception("The layer_class_dict is None when assert_dim is True. Please define the layer_class_dict")
    ...
    def find_new_edges(model, covered_edges, max_num=10, mode="diverse"):
        ...
        filtered_global_candidate_edges = ArchitectureUtils.trim_edge(global_candidate_edges, assert_dim=True, layer_class_dict=layer_class_dict)  # type: list
        ...
        if len(filtered_other_candidate_edges) != 0 and mode == "diverse":
            if len(filtered_other_candidate_edges) + len(selected_edge_list) >= num_edge:
                ...
                random_idx = np.random.choice(len(filtered_other_candidate_edges), num_edge - len(selected_edge_list), replace=False)
                for idx in random_idx:
        ...
        random_idx = np.random.choice(len(filtered_all_candidate_edges), num_edge-len(selected_edge_list), replace=False)
        for idx in random_idx:
    ...
    def choose_new_layer(model, layer_names, covered_layers, covered_edges, max_num=10, mode="diverse"):
        ...
        filtered_global_layer_sequences = filter_sequences_by_ndim(global_candidate_layer_sequence)  # {layer_right_class: [layer1, layer2]}
        filtered_local_layer_sequences = filter_sequences_by_ndim(local_candidate_layer_sequence)  # {layer_right_class: [layer1, layer2]}
        ...
        if len(filtered_global_layer_sequences) != 0 and mode == "diverse":
            ...
            right_layer_class_list = list(np.random.choice(list(filtered_global_layer_sequences.keys()), choose_num, replace=False))
        ...
        if len(selection_pair) < num_layer and len(filtered_local_layer_sequences) != 0 and mode == "diverse":
            ...
            right_layer_class_list = list(np.random.choice(list(filtered_local_layer_sequences.keys()), choose_num, replace=False))
            for right_layer_class in right_layer_class_list:
                left_layer = np.random.choice(filtered_local_layer_sequences[right_layer_class])
                selection_pair[left_layer.name] = right_layer_class
                mylogger.info('insert {} after {}'.format(right_layer_class, left_layer.name))
        if len(selection_pair) < num_layer:
            mylogger.info(f"Fail to find enough new layers to insert, randomly choose {num_layer-len(selection_pair)}")
        ...
        for layer in layer_list:
            ...
            for l in input_layers[layer.name]:
                ...
                if l in model_outputs:
                    model_outputs.pop(l)  # if the layer's output have been used, pop it out, otherwise consider it as the model's output
    ...
    def choose_layers_for_mdtype(model, input_diversity, max_num, mode="diverse"):
        ...
        if num_layer != 0:
            ...
            for layer_class in selected_layer_class_list:
                ...
                selected_dtype = np.random.choice(global_layer_class_dict[layer_class])
                selected_layer_pair[layer_name] = selected_dtype
                mylogger.info(f"No Global DType Can Be Found, Randomly Select DType: {selected_dtype} For Layer: {layer_name}")
        return selected_layer_pair
    ...
    def extract_edges(model):
        ...
        existing_edges = ArchitectureUtils.trim_edge(existing_edges)  # trim edge to remove edges that are not from the given layer.
