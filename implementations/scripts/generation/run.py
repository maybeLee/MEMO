...
class Driver(object):
    def __init__(self, flags):
        ...
        self.frameworks = {}
        self.c_libraries = {
                            "tensorflow_new": self.config["parameters"]["c_tensorflow_new"],
                            "mxnet_new": self.config['parameters']["c_mxnet_new"],
                            "pytorch": self.config['parameters']["c_pytorch"],
                            "onnx": self.config['parameters']["c_onnx"],
                            }
        self.py_libraries = {
                             "tensorflow_new": self.config['parameters']["py_tensorflow_new"],
                             "mxnet_new": self.config['parameters']["py_mxnet_new"],
    ...
    def save_api_cov(self, bk="tensorflow_new"):
        api_cov, api_pair_cov, config_cov, input_cov, ndims_cov, dtype_cov, shape_cov = self.architecture_measure.coverage()
        with open(os.path.join(self.delta_dir, bk + "_api_cov.txt"), "a") as file:
            end_time = time.time()
            file.write(f"Current time (seconds): {end_time - self.start_time}\n")
            file.write(f"api coverage result (covered api; covered api pair; covered config; covered input; covered ndims; covered dtype; covered shape):\n")
            file.write(f"{api_cov}; {api_pair_cov}; {config_cov}; {input_cov}; {ndims_cov}; {dtype_cov}; {shape_cov}\n")
    ...
    def after_prediction(self, model, status_crash_nan, accumulative_incons, reward=None):
        reward = 0.5*reward + 0.5*self.guidance.get_score()  # refine the reward: 0.3*reward+0.7*branch coverage (1, 0)
    ...
    def initiate_seed_selector(self):
        ...
        if self.seed_selector_exist():
            main_logger.info("Seed Selector Exists! Continue MEMO!")
            self.seed_selector, last_iteration = self.load_seed_selector()
        else:
            main_logger.info("Seed Selector Not Exists! Start MEMO From Beginning")
    ...
    def initiate_guidance(self):
        # MEMO calculates ARCS (Use coverage.py's module to collect the arcs in python) after each iteration
    ...
    def run(self, ):
        ...
        main_logger.info(f"MEMO Is Finished: Time used: {h} hour,{m} min,{s} sec")
