...
class LayerUtils:
    ...
    def conv1d(input_shape=None):
        ...
        layerStack = LayerStack()
        if input_shape is not None:
            layerStack = DimSolver.check_before_add(layerStack, keras.layers.Conv1D(input_shape[-1], 3, strides=1, padding='same'), input_shape, 3)
        else:
    ...
    def conv2d(input_shape=None):
        ...
        layerStack = LayerStack()
        if input_shape is not None:
            layerStack = DimSolver.check_before_add(layerStack, keras.layers.Conv2D(input_shape[-1], 3, strides=(1,1), padding='same'), input_shape, 4)
        else:
    ...
    def separable_conv_1d(input_shape=None):
        ...
        layerStack = LayerStack()
        if input_shape is not None:
            layerStack = DimSolver.check_before_add(layerStack, keras.layers.SeparableConv1D(input_shape[-1], 3, strides=1, padding='same'), input_shape, 3)
        else:
    ...
    def separable_conv_2d(input_shape=None):
        ...
        layerStack = LayerStack()
        if input_shape is not None:
            layerStack = DimSolver.check_before_add(layerStack, keras.layers.SeparableConv2D(input_shape[-1], 3, strides=(1,1), padding='same'), input_shape, 4)
        else:
    ...
    def depthwise_conv_2d(input_shape=None):
        ...
        layerStack = LayerStack()
        if input_shape is not None:
            layerStack = DimSolver.check_before_add(layerStack, keras.layers.DepthwiseConv2D(3, strides=(1,1), padding='same'), input_shape, 4)
        else:
    ...
    def conv_2d_transpose(input_shape=None):
        ...
        layerStack = LayerStack()
        if input_shape is not None:
            layerStack = DimSolver.check_before_add(layerStack, keras.layers.Conv2DTranspose(input_shape[-1], 3, strides=(1,1), padding='same'), input_shape, 4)
        else:
    ...
    def max_pooling_1d(input_shape=None):
        ...
        layerStack = LayerStack()
        if input_shape is not None:
            layerStack = DimSolver.check_before_add(layerStack, keras.layers.MaxPooling1D(pool_size=3, strides=1, padding='same'), input_shape, 3)
        else:
    ...
    def max_pooling_2d(input_shape=None):
        ...
        layerStack = LayerStack()
        if input_shape is not None:
            layerStack = DimSolver.check_before_add(layerStack, keras.layers.MaxPooling2D(pool_size=(3, 3), strides=1, padding='same'), input_shape, 4)
        else:
    ...
    def average_pooling_1d(input_shape=None):
        ...
        layerStack = LayerStack()
        if input_shape is not None:
            layerStack = DimSolver.check_before_add(layerStack, keras.layers.AveragePooling1D(pool_size=3, strides=1, padding='same'), input_shape, 3)
        else:
    ...
    def average_pooling_2d(input_shape=None):
        ...
        layerStack = LayerStack()
        if input_shape is not None:
            layerStack = DimSolver.check_before_add(layerStack, keras.layers.AveragePooling2D(pool_size=(3, 3), strides=1, padding='same'), input_shape, 4)
        else:
    ...
    def conv_lstm_2d(input_shape):
        import keras
        layerStack = LayerStack()
        layerStack = DimSolver.check_before_add(layerStack, keras.layers.ConvLSTM2D(input_shape[-1], kernel_size=(1, 1), strides=(1, 1), padding='same', return_sequences=True), input_shape, 4)
        return layerStack.get_layers()
    ...
    def cropping2d(input_shape):
        ...
        layerStack = DimSolver.check_before_add(layerStack, keras.layers.Cropping2D(cropping=((1, 1), (1, 1))), input_shape, 4)
        return layerStack.get_layers()
...
class LayerMatching:
    ...
    def cropping2d_dense(input_shape):
        ...
        actual_input_shape = DimSolver._get_output_shape(target_ndim=4, input_shape=input_shape)
        layerStack = DimSolver.check_before_add(layerStack, keras.layers.Cropping2D(cropping=((1, 1), (1, 1))), input_shape, 4)
        layerStack.add(keras.layers.Reshape(((actual_input_shape[1] - 2) * (actual_input_shape[2] - 2) * actual_input_shape[3],)))
    ...
    def upsampling_1d_dense(input_shape):
        ...
        layerStack.add(keras.layers.Dense(min(actual_input_shape[1], actual_input_shape[2])))  # the original is: actual_input_shape[1] * actual_input_shape[2], but it is too large, change to min(actual_input_shape[1], actual_input_shape[2])
        return layerStack.get_layers()
    ...
    def simple_rnn_dense(input_shape):
        ...
        layerStack = LayerStack()
        actual_input_shape = DimSolver._get_output_shape(target_ndim=3, input_shape=input_shape)
        layerStack = DimSolver.check_before_add(layerStack, keras.layers.SimpleRNN(50, input_shape=actual_input_shape[1:], return_sequences=False), input_shape, 3)
        units = actual_input_shape[1] * actual_input_shape[2]
        output_shape, units = LayerMatching.simplify_output(units, actual_input_shape)
        layerStack.add(keras.layers.Dense(units))
        layerStack.add(keras.layers.Reshape(output_shape[1:]))
