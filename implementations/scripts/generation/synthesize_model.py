...
import numpy as np
from scripts.mutation.memo_mutation_operators import copy_model_from_json
...
class ModelDeduplication(object):
    ...
    def append_layer_to_model(self, model, layer_class, required_input_shape=None):
        from scripts.mutation.memo_mutation_operators import NL_mut
        target_json = model.to_json()
        del model
        model_copy = copy_model_from_json(target_json)
        new_layer = self.total_layers[layer_class]
    ...
    def change_layer_shape(model, layer_idx):
        from scripts.mutation.memo_mutation_operators import InputMutationUtils
        target_json = model.to_json()
        del model
        model_copy = copy_model_from_json(target_json)
...
if __name__ == "__main__":
    ...
    new_model_path = "../data/working_dir/synthesized_models"
