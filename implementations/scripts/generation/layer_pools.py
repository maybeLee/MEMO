...
class ConfigurationUtils:
    ...
    def random_config(layer):
        ...
        config_pool = json.load(open("boostraps/api_implementations/api_config_pool.json", "rb+"))
...
class RNNLayers:
    def __init__(self):
        ...
        self.available_layers = {
            ...
            "ConvLSTM1D": RNNLayers.conv_lstm_1d,  # cannot apply this because ConvLSTM1D has not been implemented in Keras 2.2.4
            "ConvLSTM2D": RNNLayers.conv_lstm_2d,  # Tested
            "ConvLSTM3D": RNNLayers.conv_lstm_3d,  # TBA, this does not exist in keras 2.2.4
    ...
    def conv_lstm_1d(input_shape=None, config=None):
        import keras
        layerStack = LayerStack()
        candidate_layer = keras.layers.ConvLSTM1D(input_shape[-1], kernel_size=1, strides=1, padding="same", return_sequences=True)
        if config is None:
            inserted_layer = ConfigurationUtils.random_config(candidate_layer)
        else:
            inserted_layer = ConfigurationUtils.from_config(candidate_layer, config)
