...
def parallel_predict(args):
    ...
    if bk == "pytorch" or bk == "tensorflow_new" or bk == "mxnet_new" or bk == "onnx":
        cuda_version = "cuda-11"
        if os.environ["HOSTNAME"] == "sccpu4.cse.ust.hk" and bk == "mxnet_new":
            cuda_version = "cuda-10"
    else:
        raise ValueError("Undefined backend proposed!")
    ...
    if guidance_strategy == "ARCS" and coverage == bk:
        ...
        arcs = 0
    pref = f"source ~/anaconda3/etc/profile.d/conda.sh && conda activate {envs_name} && {envs} {python_bin} -m"
    cmd = f"{pref} scripts.prediction.predict_model --backend {bk} --model_path {model_path} --model_name {model_name} --redis_db {redis_db} --config_name {config_name} --seed 2022 --python_prefix {python_prefix} --arcs {arcs} -W ignore::DeprecationWarning && conda deactivate"
