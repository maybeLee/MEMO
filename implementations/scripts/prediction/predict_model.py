...
class Predictor(object):
    ...
    def _transform_onnx(self):
        ...
        try:
            subprocess.call(f"{envs} {self.model_convertor_python} -u -m scripts.tools.prediction_toolkit --model_path {self.model_path}",
    ...
    def _transform_pb(self):
        # transform keras model to tensorflow pb
        # if not os.path.exists(self.pb_path):
        cuda_version = "cuda-11"
        envs = f"CUDA_HOME=/usr/local/{cuda_version} CUDA_ROOT=/usr/local/{cuda_version} LD_LIBRARY_PATH=/usr/local/{cuda_version}/lib64:$LD_LIBRARY_PATH PATH=/usr/local/{cuda_version}/bin:$PATH"
        cmd = f"{envs} {self.model_convertor_python} -u -m scripts.tools.convert_h5_to_pb --model_path {self.model_path}"
        subprocess.call(cmd, shell=True)

    def _predict_pb(self):
        from tensorflow.python.platform import gfile
        import tensorflow as tf
        with tf.compat.v1.Session() as sess:
            # load model from pb file
            with gfile.FastGFile(self.pb_path, 'rb') as f:
                graph_def = tf.compat.v1.GraphDef()
                graph_def.ParseFromString(f.read())
                sess.graph.as_default()
                g_in = tf.import_graph_def(graph_def)
            # inference by the model (op name must comes with :0 to specify the index of its output)
            tensor_output = sess.graph.get_tensor_by_name(f"import/{self.model.output.name}")
            tensor_input = sess.graph.get_tensor_by_name(f"import/{self.model.input.name}:0")  # in tensorflow 2.9, the name of model input does not have ':0' part
            predictions = sess.run(tensor_output, {tensor_input: self.input})
            return predictions
    ...
    def before_predict(self, ):
        ...
        if self.bk == "tensorflow_new":
            ...
            # self._transform_pb()  # convert keras model to pb

        elif self.bk == "mxnet_new":
            os.environ["MXNET_ENGINE_TYPE"]="NaiveEngine"
            os.environ["MXNET_SUBGRAPH_VERBOSE"] = "0"
            os.environ["MXNET_CUDNN_AUTOTUNE_DEFAULT"] = "0"
            os.environ["MXNET_CUDNN_LIB_CHECKING"] = "0"
            os.environ["MXNET_USE_OPERATOR_TUNING"] = "0"  # add this flag so mxnet's import stage will have low cpu load
            if len(self.gpu_list) >= 2:
    ...
    def predict(self, ):
        if self.bk in ["tensorflow_new", "mxnet_new"]:
            if self.bk == "mxnet_new":
    ...
    def after_predict(self, ):
        del self.model
        if self.bk == "tensorflow_new" and self.arcs == 1:
            cov.stop()
            covData = cov.get_data()
            covBranch = [(files, sorted(covData.arcs(files))) for files in covData.measured_files()]
            import json
            tf_file_br = json.load(open("scripts/analysis/tf_file_br.json", "rb+"))
            arcs = {}
            for i in covBranch:
                file_path = i[0]
                if file_path not in tf_file_br:
                    self.main_logger.info(f"COVERAGE ERROR! THE FILE IS NOT CONSIDERED: {file_path}")
                    continue
                arc = i[1]
                filtered_arc = []
                for ar in arc:
                    l, r = ar
                    if str(l) in tf_file_br[file_path]:
                        # we only consider the arcs that are branch
                        filtered_arc.append(f"{l}_{r}")
                if len(filtered_arc) == 0:
                    continue
                arcs[file_path] = filtered_arc
...
if __name__ == "__main__":
    ...
    if flags.backend == "tensorflow_new" and flags.arcs == 1:
